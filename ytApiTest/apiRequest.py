...
class InterFaceReq():
	...
	def get_interface_cookie(self, url: str, host_key=None):
		...
		if response.request._cookies:
			...
			if response.json().__contains__('data') and isinstance(response.json().get('data'), dict):
				if response.json()['data'].__contains__('userinfo'):
					headers = {'User-Agent': 'python-requests/2.22.0',
					           ...
					           'Cookie': 'userId={userId}; '
					                     'sessionId={sessionId}; '
					                     'SMclient=MicroMessenger; '
					                     'SMmodel=Xiaomi_MI_8; '
					                     'SMsystem=Android_8.1.0; '
					                     'SMver=7.0.3; SMdisplay=393x818; '
					                     'SDKVersion=2.6.1; '
...
if __name__ == '__main__':
	r = requests.get('http://mis.internal.fat.supermonkey.com.cn/mis/index.html')
	r.raise_for_status()
	requests.delete(r.url)
